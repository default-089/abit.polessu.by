...
$alert_html = '';
if (isset($_GET['searchAbit'])) {
	$user = R::findOne('users', 'email = ?', array($_GET['searchAbit']));
	if ($user) { //если пользователь с таким email существет
		...
		$alert_html = '<div class="alert alert-danger pl-lg-5" role="alert">Пользователь с таким email не найден!</div>';
	}
}
?>
<div class="wrap">
...
if ($user) {
	echo '
	<pre>
		<h4>Информация об абитуриенте:</h4>',
		print_r($applicants),
		'<hr>
		<h4>Информация о родителях абитуриента:</h4>',
		print_r($parents),
		'<hr>
		<h4>Информация о выбранных специальностях:</h4>',
		print_r($contest),
	'</pre>';
}
?>
	...
	<div class="footer"><?php include_once 'layout/footer.html'; ?></div>
</div>
